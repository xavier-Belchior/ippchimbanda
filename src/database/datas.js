export const empresa = [
  {
    nome: "Instituto P.P Chimbanda",
    lema: "Ensino de qualidade a preços baixos",
    img: "img/Ilustra.png",
    sobre: `No nosso site institucional, você encontrará
           oportunidades excepcionais para impulsionar o seu crescimento pessoal e
           profissional e Junte-se a nós e dê o primeiro passo em direção a um futuro brilhante e repleto de
           realizações....,`,
  },
  {
    alunosAlbergar: 200,
    professores: 23,
    cursos: 3,
    estudantesFormados: 300,
    sobreChimbanda: "Sobre Chimbanda",
    paragrafo: `Bem-vindo ao Chimbanda, que se dedica a fornecer uma educação excepcional e inspiradora para jovens
            no Lobito. Com um compromisso inabalável com a excelência acadêmica, inovação e atendimento
            individualizado.A nossa missão é formar profissionais competentes nas áres de Tecnologias, Administração e Serviços: homens capazes de exercer plenamente a cidadania .`,
    imgEmpresa: "img/imgabout2.png",
    modalNameSobre: "Instituto Politecnico Privado Chimbanda",
    modalParagrafoSobre: `O Instituto Politécnico Privado Chimbanda é uma instituição de ensinio vocacionado
    a leccionar cursos técnicos do I e II ciclo, está localizado na província de Benguela, Município do Lobito Bairro do alto 
    Esperança, perto do término dos táxis.

    O Instituto é propriedade da ACNET, LDA, pessoa colectiva registada na conservatoria do registro  comercial de Benguela contribuiente fiscal nº5112159839.
    Começou a funcionar no dia 10 de Fevereiro de 2023 com 3 turmas e em dois periodos, tarde e noite.
    </br>
    Agora o instituto tem 6 turma e 1Laboratorio e dois periodos de manhã e tarde, mas de 100alunos-albergado e com 23professores.  

     `,
  },
];
export const empresaCursos = [
  {
    img: "img/i.t.jpeg",
    name: "Tecnico de Informatica de Gestão",
    paragrafo: `Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
         tempor incididunt ut labore et dolore magna aliqua.`,
    alt: "foto do curso de Informatica",
  },
  {
    img: "img/r.h.jpeg",
    name: "Tecnico de Gestão de Recursos Humanos",
    paragrafo: `Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
         tempor incididunt ut labore et dolore magna aliqua.`,
    alt: "foto do curso de recursos humanos",
  },
  {
    img: "img/g.e.jpeg",
    name: "Tecnico de Gestão de Empresarial",
    paragrafo: `Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod
         tempor incididunt ut labore et dolore magna aliqua.`,
    alt: "foto do curso de Gestão de empresarial",
  },
];
export const modalCursos = [
  {
    name: "Tecnico de Informatica de Gestão",
    saidaOne: "Responsavel da área de Informática nas pequenas empresa.",
    saidaTwo: "Gestor de Sistemas Informáticos.",
    saidaThree: "Administrador de base de dados.",
  },
  {
    name: "Tecnico de Gestão de Recursos Humanos",
    saidaOne: "Consultoria de Recursos Humanos.",
    saidaTwo: "Recrutamento e seleção.",
    saidaThree: "Gestão de carreira.",
    saidaFour: "Gestão de Pequenas empresas.",
    saidaFive: "Gestão Financeira.",
  },
  {
    name: "Tecnico de Gestão Empresarial",
    perfilSaida: "Saidas Profissionais",
    saidaOne: "Direcção de pequenas empresas.",
    saidaTwo: "Direcção comercial e Marketing.",
    saidaThree: "Consultoria de empresas.",
    saidaFour: "Gestão Financeira",
  },
];

export const propinaTenClasses= [
  {
    name:"Tecnico de Informatica de Gestão",
    valor:5500,
    multa:1650,
    img:"img/icon-propinas/1.png"
},  {
  name:"Tecnico de Gestão de Recursos Humanos",
  valor:5500,
  multa:1650,
  img:"img/icon-propinas/2.png"
},
{
  name:"Tecnico de Gestão Empresarial",
  valor:5500,
  multa:1650,
  img:"img/icon-propinas/3.png"
},

];

export const propinaElevenClasses= [
  {
    name:"Tecnico de Informatica de Gestão",
    valor:6000,
    multa:1800,
    img:"img/icon-propinas/1.png"
},  
{
  name:"Tecnico de Gestão de Recursos Humanos",
  valor:6000,
  multa:1800,
  img:"img/icon-propinas/2.png"
},
{
  name:"Tecnico de Gestão Empresarial",
  valor:6000,
  multa:1800,
  img:"img/icon-propinas/3.png"
},

];
export const propinaTwelveClasses= [
  {
    name:"Tecnico de Informatica de Gestão",
    valor:6500,
    multa:1950,
    img:"img/icon-propinas/1.png"
},  {
  name:"Tecnico de Gestão de Recursos Humanos",
  valor:6500,
  multa:1950,
  img:"img/icon-propinas/2.png"
},
{
  name:"Tecnico de Gestão Empresarial",
  valor:6500,
  multa:1950,
  img:"img/icon-propinas/3.png"
},

];
export const propinaThirteenClasses= [
  {
    name:"Tecnico de Informatica de Gestão",
    valor:7000,
    multa:2100,
    img:"img/icon-propinas/1.png"
},  {
  name:"Tecnico de Gestão de Recursos Humanos",
  valor:7000,
  multa:2100,
  img:"img/icon-propinas/2.png"
},
{
  name:"Tecnico de Gestão Empresarial",
  valor:7000,
  multa:2100,
  img:"img/icon-propinas/3.png"
},

];
export const uniformes=[
  {
    name:"Masculino",
    foto:"img/uniformes/f-blue.png",
    alt:"foto de uniforme feminino azul"
  },
  {
    name:"Feminino",
    foto:"img/uniformes/m-blue.png",
    alt:"foto de uniforme  masculino azul"
  },
  {
    name:"Masculino",
    foto:"img/uniformes/f-yellow.png",
    alt:"foto de uniforme feminino  amarelo"
  },
  {
    name:"Masculino",
    foto:"img/uniformes/m-yellow.png",
    alt:"foto de uniforme masculino amerlo"
  },
]
export const testemunhas=[
  {
    name:"Severiano Sebastião",
    paragrafo:`Lorem ipsum dolor sit amet consectetur adipisicing elit. Iste, delectus. Mollitia quod, vero
    obcaecati hic maxime fuga excepturi aliquid dolore architecto quas debitis culpa sed id,
    sint officiis neque incidunt voluptatem .`,
    img:"team/1.jpg",
    alt:"fotos de testemunhas"
  },
  {
    name:"Ana Santos",
    paragrafo:`Lorem ipsum dolor sit amet consectetur adipisicing elit. Iste, delectus. Mollitia quod, vero
    obcaecati hic maxime fuga excepturi aliquid dolore architecto quas debitis culpa sed id,
    sint officiis neque incidunt voluptatem .`,
    img:"team/2.jpg",
    alt:"fotos de testemunhas"
  },
  {
    name:"Lucas Gabriel",
    paragrafo:`Lorem ipsum dolor sit amet consectetur adipisicing elit. Iste, delectus. Mollitia quod, vero
    obcaecati hic maxime fuga excepturi aliquid dolore architecto quas debitis culpa sed id,
    sint officiis neque incidunt voluptatem .`,
    img:"team/3.jpg",
    alt:"fotos de testemunhas"
  }
]
const facultys = [
  {
    name: "Jose Melban",
    img: "img/teachers/teacher06.jpg",
    alt: "Jose Melban",
    subject: "Ui design",
    whatsapp: 956351406,
  },
  {
    name: "Ana Milly",
    img: "img/teachers/teacher03.jpg",
    alt: "Ana Milly",
    subject: "Marketing",
    whatsapp: 955195350,
  },
  {
    name: "Diogo Billie",
    img: "img/teachers/teacher04.jpg",
    alt: "Diogo Billie",
    subject: "Economy",
    whatsapp: 955195350,
  },
  {
    name: "Christopher Xavier",
    img: "img/teachers/teacher05.jpg",
    alt: "Christopher Xavier",
    subject: "Design",
    whatsapp: 955195350,
  },
  {
    name: "Suzana Fell",
    img: "img/teachers/teacher02.jpg",
    alt: "Suzana Fell",
    subject: "UX Design",
    whatsapp: 955195350,
  },
  {
    name: "Jenny Mill",
    img: "img/teachers/teacher01.jpg",
    alt: "Jenny Mill",
    subject: "Art",
    whatsapp: 955195350,
  },
];
export default facultys;
export const events=[
  {
    name:"Batismo dos calouros",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/batismo.jpg",
    alt:"foto de batismos dos calouros"
  },
  {
    name:"Cerimonia dos vinalistas",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/cerimoniafinalistas.jpg",
    alt:"foto de cerimoniania dos finalistas"
  },
  {
    name:"Mediateca e Museu-Benguela",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/mediateca e museu.jpg",
    alt:"foto de mediateca e museu de Benguela"
  },
  {
    name:"Batismo dos calouros",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/batismo.jpg",
    alt:"foto de batismos dos calouros"
  },
  {
    name:"Cerimonia dos vinalistas",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/cerimoniafinalistas.jpg",
    alt:"foto de cerimoniania dos finalistas"
  },
  {
    name:"Mediateca e Museu-Benguela",
    paragrafo:`
    é um dia especial dedicado a homenagear e valorizar as contribuições dos professores na
    sociedade.as
    contribuições dos professores na sociedade.`,
    img:"img/events/mediateca e museu.jpg",
    alt:"foto de mediateca e museu de Benguela"
  }
]
export const equipa=[
  {
    name:"Herculano Bunju",
    sector:"Chefe da Secretária",
    img:"team/1.png",
    alt:"A foto do secretario da escola"
  },
  {
    name:"Avelino Chimbanda",
    sector:"Director",
    img:"team/2.png",
    alt:"A foto do director da escola"
  },
  {
    name:"Frederico chingando Lucas",
    sector:"Subdirector Pedágogico",
    img:"team/3.png",
    alt:"A foto do subdirector Pedágogico"
  }
]